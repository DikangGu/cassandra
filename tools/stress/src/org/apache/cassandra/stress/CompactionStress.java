...
public abstract class CompactionStress implements Runnable
{
    ...
    ColumnFamilyStore initCf(StressProfile stressProfile, boolean loadSSTables)
    {
        ...
        // generate tokens for the vnodes in this cluster
        generateTokens(stressProfile.seedStr, StorageService.instance.getTokenMetadata(), numTokens);

        // generate the keyspace if necessary
        if (Schema.instance.getKSMetaData(stressProfile.keyspaceName) == null)
            Schema.instance.load(stressProfile.createKSMetaDataFromStatement());

        // set up the column family store with specified directories
        CFMetaData cfMetaData = stressProfile.createCFMetaDataFromStatement();
        List<File> dataDirectories = getDataDirectories();
        ColumnFamilyStore cfs = CQLSSTableWriter.Builder.createOfflineTable(cfMetaData, dataDirectories);
        if (loadSSTables)
        {
    ...
    /**
     * Loads stress profile from yaml file.
     */
    ...
    public static class Compaction extends CompactionStress
    {
        ...
        /**
         * Compact the CFS data fully. Wait until there are no more active compactions or workers.
         */
        public void run()
        {
            // Setup
            ...
            cfs.compactionStrategyWrapper.compactionLogger.enable();
            ...
            else
            {
                ...
                cfs.compactionStrategyWrapper.enable();
                for (int i = 0; i < threads; i++)
                    futures.addAll(CompactionManager.instance.submitBackground(cfs));
            }

            long working = countIncompleteFutures(futures);

            // Report compaction stats while working
            while (stillWorking(working))
            {
                // Re-up any bg jobs
                if (!maximal)
                {
                ...
                working = countIncompleteFutures(futures);
            }
        ...
        private boolean stillWorking(long working)
        {
            return working > 0 || CompactionManager.instance.getActiveCompactions() > 0;
        }

        private int countIncompleteFutures(List<Future<?>> futures)
        {
            int count = 0;
    ...
    public static class DataWriter extends CompactionStress
    {
        ...
        /**
         * Write to CFS directory until we have written the specified amount.
         * The amount we write won't be the exact amount specified because the threads won't terminate
         * exactly when the amount if reached. But it will be close enough.
         */
        public void run()
        {
            StressProfile stressProfile = getStressProfile();
            ...
            final WorkManager workManager = new WorkManager.FixedWorkManager(Long.MAX_VALUE);
